...
def fenced_code_blocks(markdown_filename: str) -> List[str]:
    ...
    markdown_filename : Path to the Markdown file.

    Returns a list of strings, one for the contents of each Markdown
        fenced code block.
    """
...
def fenced_block_nodes(fp: IO[str]) -> List[commonmark.node.Node]:
    ...
    Returns a list of commonmark.node.Node objects.
    """
    nodes = []
    doc = fp.read()
